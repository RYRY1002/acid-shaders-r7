...
//#define VolumetricClouds // Enables or Disables Volumetric Clouds.
...
const float 	shadowIntervalSize 		= 0.001;
...
uniform vec4  viewSpacePosition;
...
float CalculateSunlightVisibility(in vec4 viewSpacePosition, in float directShading) {
	...
	vec4 position = vec4((texture2D(colortex6, texcoord.st).xyz), 1.0);
	...
	float shading = shadow2D(shadow, vec3(position.xyz)).x;
	 ...
	 float dist = sqrt((position.x / 6) * (position.x / 6) + (position.y / 6) * (position.y / 6));

	 float diffthresh = dist * 2.0f + 0.0f;
			 diffthresh *= 1f / (shadowMapResolution / 1024.0f);
			 //diffthresh /= shadingStruct.direct + 0.1f;

	#if defined ENABLE_SOFT_SHADOWS

				int count = 0;
				float spread = 0.25f / shadowMapResolution;

				for (float i = -3.0f; i <= 3.0f; i += 1.0f) {
					for (float j = -3.0f; j <= 3.0f; j += 1.0f) {
						 shading += shadow2D(shadow, vec3(position.st + vec2(i, j) * spread, position.z - 0.0018f * diffthresh)).x;
						count += 1;
					}
				}
				shading /= count;
...
void 	CalculateAtmosphericScattering(inout vec3 color, in SurfaceStruct surface)
{
	...
	fogFactor *= pow(eyeBrightnessSmooth.y / 190.0f, 1.0f);
...
vec4 CloudColor(in vec4 worldPosition, in float sunglow, in vec3 worldLightVector)
{
	...
	float cloudHeight = 425f;
	float cloudDepth  = 270.0f;
	...
	else
	{
		...
		float cloudAltitudeWeight = 1.0f - clamp(distance(worldPosition.y, cloudHeight) / (cloudDepth / 2.0f), 0.0f, 1.0f);
			  ...
			  cloudAltitudeWeight = pow(cloudAltitudeWeight, mix(0.43f, 0.8f, rainStrength));
		...
		float cloudAltitudeWeight2 = 1.0f - clamp(distance(worldPosition.y + worldLightVector.y * lightOffset * 150.0f, cloudHeight) / (cloudDepth / 2.0f), 0.0f, 1.0f);
			  ...
			  cloudAltitudeWeight2 = pow(cloudAltitudeWeight2, mix(0.43f, 0.8f, rainStrength));
	...
	float directLightFalloff = mix(firstOrder, secondOrder, 0.6f);
...
void CloudPlane(inout SurfaceStruct surface)
{
	...
	float cloudsAltitude = 740.0f;
	float cloudsThickness = 150.0f;

	float cloudsUpperLimit = cloudsAltitude + cloudsThickness * 0.0f;
	float cloudsLowerLimit = cloudsAltitude - cloudsThickness * 0.0f;
...
void main() {
	...
	shading.direct  			= CalculateDirectLighting(NdotL, mask);				//Calculate direct sunlight without visibility check (shadows)
	...
	CalculateAtmosphericScattering(finalComposite.rgb, surface); // TBC
